...
- **Capability-gated controls.** Start/stop/restart/uninstall/install render only when the transport reports that launchctl is reachable. Otherwise the Install action degrades to *Copy manifest*, so the UI stays honest instead of offering a button that cannot work.
